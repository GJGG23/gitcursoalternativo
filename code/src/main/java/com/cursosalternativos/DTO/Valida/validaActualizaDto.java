...
public class validaActualizaDto {
	
	public responseErrorMsjDto validaactualizacion(requestActualizaMateriaDto request) {
		...
		if(request.getIdalumno() < 1) {
			...
			msjerror.setDetalle("la peticion debe de tener idalumno valido para poder proseguir con la actualizacion");
			return msjerror;
		}
		...
		if(request.getIdmateria() < 1) {
			...
			msjerror.setDetalle("la peticion debe de tener idmateria valido para poder proseguir con la actualizacion");
			return msjerror;
		}
		...
		if(request.getCalificacion() < 1) {
			...
			msjerror.setDetalle("la peticion debe de tener una calificacion valida para poder proseguir con la actualizacion");
			return msjerror;
		}
